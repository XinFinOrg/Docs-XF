# XinFin Network Proposal for Decentralized Applications-Oriented Proof-of-Stake Sharding Architecture
...
We are working to create an alternative, scalaftble financial system which is more secure, transparent, efficient, inclusive and equitable for everyone.
...
This document describes XinFin Network’s initial proposal for decentralized applications-oriented Proof-of-Stake Sharding Architecture. 
...
### Abstract
This paper proposes a sharding architecture solution for the XinFin Network public blockchain infrastructure.
Besides the aims at significantly improving the transaction processing performance in the current XinFin Network design with the Proof-of-Stake Voting (PoSV) consensus, the presented sharding is designed in order to support decentralized applications’ performance. 
...
### Keywords
Sharding, Blockchain, Smart contract, XinFin Network, cross-shard transactions, randomization, data availability,security, PoSV, consensus.
...
In this paper, we propose a sharding architecture for XinFin Network but can be applied to any Proof-of-Stake-based blockchain systems to address the aforementioned issues. 
...
However, to better describe the sharding architecture, we summarize some important information about XinFin Network’s PoSV.

Specifically, XinFin Network features a voting-based system to elect 150 Masternodes that are responsible for creating, verifying and finalizing the blocks created within a period, called epoch, which lasts for 900 of two-second block-times. 
Each coin-holder must deposit at least 50 000 Xinfin to a Voting smart contract and must satisfy a set of infrastructure requirements. 
...
XinFin Network features a Voting smart contract that is for masternode candidates and coin-holders to deposit and vote for masternodes, respectively. 
In the sharding architecture, this smart contract is deployed on the root chain. 
Coin-holders interact with the Voting smart contract in the current XinFin Network as they interact with the root chain in the sharding architecture. 
...
Ideally, all masternodes are responsible for the root chain, just as they are maintaining XinFin Network in PoSV. 
...
As a reminder, masternodes in XinFin Network in an epoch is selected at the end of the previous epoch and there is also a randomization process during the previous epoch in order to select block verifiers for double validation. 
...
We utilize the decentralized randomization algorithm that was previously introduced in our XinFin Network Proof-of-Stake Voting consensus. 
...
Each shard sends their created blocks, block hashes and block signatures to the root chain that stores them in the XinFin Network block signer smart contract (see Section Consensus protocol for more information).

### Consensus protocol

In PoSV, there are currently two consensus smart contracts deployed onto XinFin Network:
...
In order to alleviate this complicated account management issue, we intend to integrate an account management layer into our Xinfin wallet. 
This layer is to help users manage their accounts in different shards as a single account in the current XinFin Network design. 
...
However, the issue can be alleviated if the scheme is implemented along with a near-zero fee transaction system such as XinFin Network.
...
In order to maintain fast confirmation time as in the current XinFin Network PoSV design and to have the shards’ liveness, our approach is driven by an incentive and mutual verification game. 
...
The proposed solution uses the Proof-of-Stake Voting (PoSV) efficient consensus protocol proposed by Xinfinchain for intra-shard consensus. 
...
In future, once the analysis will have been done, we will implement the proposed architecture on top of the XinFin Network blockchain.
...
[10] 	Xinfinchain R&D Team, “Xinfinchain: Masternodes Design Technical White Paper Version 1.0,” Technical White Paper, 2018.
